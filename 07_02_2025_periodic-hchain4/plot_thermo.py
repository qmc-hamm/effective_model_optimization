...
def plot_thermo(dirname: str, fnames: list[str], inference_names: list[str], abinitio_names: list[str], parameters: tuple[list[str], list[str]]) -> str:
    ...
    for i, name in enumerate(inference_names):
        ...
        df_model_whole = pd.DataFrame(model_energies)
        ...
        for r in np.unique(df_model_whole.r):
            ...
            thermo_df.to_csv(f"plots/thermo_df_r{r}.csv") # todo: collate with respect to r later, and collate with respect to inference names
            mlflow.log_artifact(f"plots/thermo_df_r{r}.csv")
            ...
            plt.ylabel("Entropy")
            plt.xlim(0, 10)
            plt.savefig(f"plots/T_S_zoomed_r{r}.png", dpi=300)
            mlflow.log_artifact(f"plots/T_S_zoomed_r{r}.png")
            plt.figure()
            ...
            plt.xlim(0, 10)
            plt.savefig(f"plots/T_U_zoomed_r{r}.png", dpi=300)
            mlflow.log_artifact(f"plots/T_U_zoomed_r{r}.png")
            plt.figure()
            ...
            plt.xlim(0, 10)
            plt.savefig(f"plots/T_C_zoomed_r{r}.png", dpi=300)
            mlflow.log_artifact(f"plots/T_C_zoomed_r{r}.png")
            plt.figure()
