...
def plot_model(dirname: str, fnames: list[str], inference_names: list[str], parameters: tuple[list[str], list[str]]) -> str:
    ...
    sns.scatterplot(data=df, x='CV iteration', y='Spectrum RMSE Train (eV)', label="Train")
    sns.scatterplot(data=df, x='CV iteration', y='Spectrum RMSE Train (eV)', label="Validation")
    for inference_name in inference_names:
        #sns.lineplot(data=df, x='r (Bohr)', y=f'Spectrum RMSE Test {inference_name} (eV)', label=f"Test {inference_name}")
        sns.scatterplot(data=df, x='CV iteration', y=f'Spectrum RMSE Test {inference_name} (eV)', label=f"Test {inference_name}")
    ...
    for parameter in (parameters[0] + parameters[1]):
        ...
        sns.scatterplot(data=df, x='CV iteration', y=f'RDMD {parameter} (eV)', label="RDMD")
        #sns.lineplot(data=df, x='r (Bohr)', y=f'DMD {parameter} (eV)', label="DMD")
        sns.scatterplot(data=df, x='CV iteration', y=f'DMD {parameter} (eV)', label="Initialization")
