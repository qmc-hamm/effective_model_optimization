...
def CV_evaluate_loss(
    ...
    #print("Modified energies", dist_energy*boltzmann_weights.values[:,np.newaxis])
    #print("Original energies", dist_energy)
...
def compute_boltzmann_weights(energies, beta:float):
    """
    temperature in eV
    """
    boltzmann = np.exp(-beta*energies)
    Z = np.sum(boltzmann)
    boltzmann = boltzmann/Z
    return boltzmann/np.max(boltzmann) # Max is now 1 
...
def setup_train(
    ...
    for r in (train_rs):
        ...
        boltzmann_weights_rs[f'r{r}'] = compute_boltzmann_weights(ai_df["energy"], beta)
    ...
    for i, r in enumerate(train_rs):
        for j, param in enumerate(onebody_params + twobody_params):
            ...
            if param == 't_1':
                dmd_train_rs_params[j][i] = np.random.uniform(-4, 4, 1)
            if param == 'doccp':
                dmd_train_rs_params[j][i] = np.random.uniform(0, 12, 1)
