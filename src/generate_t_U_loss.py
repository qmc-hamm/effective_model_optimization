...
def generate_t_U(tmin:float, tmax:float, Umin:float, Umax:float, eps:float=-0.5, ngrid:int=10, fname:str="loss_grid.csv"):
    """_summary_
    ...
    fname : str, optional
        _description_, by default "loss_grid.csv"
    """
    ...
    ai_df, beta, nroots, weights, onebody, twobody, matches = return_set_up_h4(
    ...
    # for loops through t and U variables
    loss_grid = []
    for t in np.linspace(tmin, tmax, num=ngrid):
        ...
        for U in np.linspace(Umin, Umax, num=ngrid):
            ...
            #new_data["para_w_0"] = data["para_w_0"] 
...
if __name__ == "__main__":
    ngrid = 50
    #eps = -0.5
    #generate_t_U(0.0, -0.05, 0.1, 0.4, eps=eps, ngrid=ngrid, fname=f"loss_grid{ngrid}_eps{eps}.csv")
